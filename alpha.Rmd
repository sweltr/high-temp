...
where `x` is the sample by ASV table, `i` is the q-value corresponding to the metric of interest and `tree` is an ultrametric formatted phylogenetic tree (see below).
...
ssu18_norm_res$dataset <- stringr::str_replace(ssu18_norm_res$dataset, "^pit$", "pime_tree") %>%
                          stringr::str_replace(., "^pet$", "perfect_tree") %>%
                          stringr::str_replace(., "^pi$", "pime") %>%
                          stringr::str_replace(., "^pe$", "perfect")

ssu18_norm_res <- ssu18_norm_res %>% 
  dplyr::rename(c("pval_shap" = 3, "pval_bart" = 4, "test" = 5))
knitr::kable(ssu18_norm_res)
```
:::
...
```{r, eval=TRUE, echo=FALSE}
ssu18_norm_res_o <- data.frame()
for (i in colnames(ssu18_div_tab_otu[,7:ncol(ssu18_div_tab_asv)])) {
  ...
  tmp_df <- data.frame(i, tmp_shap_p, tmp_bart_p, tmp_test)
  ssu18_norm_res_o <- dplyr::bind_rows(ssu18_norm_res_o, tmp_df)
  rm(list = ls(pattern = "tmp_"))
}
ssu18_norm_res_o[,1] <- gsub('_exp', '', ssu18_norm_res_o[,1])
ssu18_norm_res_o <- ssu18_norm_res_o %>% separate(col = 1, 
                                              into = c("metric", "dataset"), 
                                              sep = "_", remove = TRUE)
ssu18_norm_res_o <- ssu18_norm_res_o %>% tidyr::replace_na(list(dataset = "full"))
ssu18_norm_res_o$dataset <- stringr::str_replace(ssu18_norm_res_o$dataset, "^pit$", "pime_tree") %>%
                          stringr::str_replace(., "^pet$", "perfect_tree") %>%
                          stringr::str_replace(., "^pi$", "pime") %>%
                          stringr::str_replace(., "^pe$", "perfect")

ssu18_norm_res_o <- ssu18_norm_res_o %>% 
  dplyr::rename(c("pval_shap" = 3, "pval_bart" = 4, "test" = 5))
knitr::kable(ssu18_norm_res_o)
```
:::
:::
:::
...
where `x` is ASV by sample table, `i` is the q-value corresponding to the metric of interest, `hier` is the hierarchy matrix, `tree` is an ultrametric formatted phylogenetic tree (see below), and `posthoc` indicates whether to run post hoc pairwise analyses.
...
e perform each analysis against the full data set, the PIME filtered data set, and the PERfect filtered data set using the function `hill_div`.
...
Next, we run the analysis for all three metrics on the data sets.
...
its18_norm_res <- its18_norm_res %>% separate(col = 1, 	
                                              into = c("metric", "dataset"), 	
                                              sep = "_", remove = TRUE)	
its18_norm_res <- its18_norm_res %>% tidyr::replace_na(list(dataset = "full"))	
its18_norm_res$dataset <- stringr::str_replace(its18_norm_res$dataset, "^pit$", "pime_tree") %>%	
                          stringr::str_replace(., "^pet$", "perfect_tree") %>%	
                          stringr::str_replace(., "^pi$", "pime") %>%	
                          stringr::str_replace(., "^pe$", "perfect")	
...
```{r, eval=TRUE, echo=FALSE}	
its18_norm_res_o <- data.frame()	
for (i in colnames(its18_div_tab_otu[,7:ncol(its18_div_tab_asv)])) {	
  ...
  tmp_df <- data.frame(i, tmp_shap_p, tmp_bart_p, tmp_test)	
  its18_norm_res_o <- dplyr::bind_rows(its18_norm_res_o, tmp_df)	
  rm(list = ls(pattern = "tmp_"))	
}	
its18_norm_res_o[,1] <- gsub('_exp', '', its18_norm_res_o[,1])	
its18_norm_res_o <- its18_norm_res_o %>% separate(col = 1, 	
                                              into = c("metric", "dataset"), 	
                                              sep = "_", remove = TRUE)	
its18_norm_res_o <- its18_norm_res_o %>% tidyr::replace_na(list(dataset = "full"))	
its18_norm_res_o$dataset <- stringr::str_replace(its18_norm_res_o$dataset, "^pit$", "pime_tree") %>%	
                          stringr::str_replace(., "^pet$", "perfect_tree") %>%	
                          stringr::str_replace(., "^pi$", "pime") %>%	
                          stringr::str_replace(., "^pe$", "perfect")	
	
its18_norm_res_o <- its18_norm_res_o %>% 	
  dplyr::rename(c("pval_shap" = 3, "pval_bart" = 4, "test" = 5))	
knitr::kable(its18_norm_res_o)	
```	
:::	
:::	
:::
