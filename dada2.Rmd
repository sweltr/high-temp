...
description: |
  DADA2 workflow for processing the **2018 16S rRNA** and the **2018 ITS** high temperature data sets. Workflow uses paired end reads, beginning with raw fastq files, ending with sequence and taxonomy tables.
author:
#  - name: Jarrod J Scott
...
This workflow is how we processed our 16S rRNA and ITS data sets using [DADA2](https://benjjneb.github.io/dada2/) [@callahan2016dada2] from samples collected in **2018**. Workflow construction is based on the [DADA2 Pipeline Tutorial (1.8)](https://benjjneb.github.io/dada2/tutorial_1_8.html) and the primer identification section of the [DADA2 ITS Pipeline Workflow (1.8)](https://benjjneb.github.io/dada2/ITS_workflow.html).

<aside>
Because the workflows share many common variable names, you must either split the workflow into a script for each data set OR run the command `remove(list = ls())` before beginning the next workflow.
</aside>
...
Command line parameters: -g GTGCCAGCMGCCGCGGTAA -a ATTAGAWACCCBDGTAGTCC -m 20 -n 2 -e 0.12 -o RAW/cutadapt/P1_R1.fastq.gz RAW/filtN/P1_R1.fastq.gz
```
...
```{r echo=FALSE, eval=TRUE}
cut_t <- read.table("files/dada2/tables/ssu18_cut.txt",
                       header = TRUE)
...
```{r, echo=FALSE, layout="l-body", warning=FALSE, fig.height=2, eval=TRUE}
...
```
<small><em>Aggregated quality score plots for forward reads.</em></small>
...
```{r, echo=FALSE, eval=TRUE}
out <- read.table("files/dada2/tables/ssu18_out_T.txt",
                       header = TRUE)
...
```{r, echo=FALSE, layout="l-body-outset", eval=TRUE}
system("cp files/dada2/figures/ssu18_plot_errorF_1.png include/dada2/ssu18_plot_errorF_1.png")
knitr::include_graphics("include/dada2/ssu18_plot_errorF_1.png")
```
...
*Total number of reads, total number of ASV, minimum and maximum ASVs, followed by the number of singletons, doubletons, etc.*
```{r, echo=FALSE, layout="l-body", eval=TRUE}
dada_pool_comp <- read.table("files/dada2/tables/ssu18_dada_compare.txt",
                        header = TRUE, sep = "\t", row.names = 1)
knitr::kable(dada_pool_comp)
```

*Total number of reads by sample.*
```{r, echo=FALSE, layout="l-body", eval=TRUE}
dada_pool_reads <- read.table("files/dada2/tables/ssu18_dada_compare_final_reads.txt",
                        header = TRUE, sep = "\t", row.names = 1)
knitr::kable(dada_pool_reads)
```

*Total number of ASVs by sample.*
```{r, echo=FALSE, layout="l-body", eval=TRUE}
dada_pool_asvs <- read.table("files/dada2/tables/ssu18_dada_compare_final_asvs.txt",
                        header = TRUE, sep = "\t", row.names = 1)
knitr::kable(dada_pool_asvs)
```
</details>
...
```{r, echo=FALSE, layout="l-body-outset", eval=TRUE}
#X# NOTE. For some reason this datatable was being given the same
#X# elementId as the first table. So I had to add elementId
trackT <- read.table("files/dada2/tables/ssu18_read_changes.txt")
datatable(trackT, width = "100%", escape = FALSE,
          elementId = "5becfm0ylbqxfrglu40z",
          extensions = 'Buttons', options = list(
            scrollX = TRUE,
            dom = 'Blfrtip',
            buttons = c('copy', 'csv', 'excel'),
...
```{r save_image_part_11, echo=FALSE}
...
```
This is cutadapt 2.8 with Python 3.7.6
Command line parameters: -g CTTGGTCATTTAGAGGAAGTAA -a GCATCGATGAAGAACGCAGC -G GCTGCGTTCTTCATCGATGC -A TTACTTCCTCTAAATGACCAAG -m 20 -n 2 -o RAW/cutadapt/P1_R1.fastq.gz -p RAW/cutadapt/P1_R2.fastq.gz RAW/filtN/P1_R1.fastq.gz RAW/filtN/P1_R2.fastq.gz
```
...
```{r, echo=FALSE, eval=TRUE}
out <- read.table("files/dada2/tables/its18_cut.txt",
                       header = TRUE)
datatable(out, width = "80%",
          extensions = 'Buttons', options = list(
            scrollX = TRUE,
...
```{r, echo=FALSE, layout="l-body-outset", warning=FALSE, fig.height=3, eval=TRUE}
...
```{r, echo=FALSE, eval=TRUE}
out_T <- read.table("files/dada2/tables/its18_out.txt",
                       header = TRUE)
...
```{r, echo=FALSE, layout="l-body-outset", eval=TRUE}
#system("cp files/dada2/figures/its18_plot_errorF_1.png include/dada2/its18_plot_errorF_1.png")
knitr::include_graphics("include/dada2/its18_plot_errorF_1.png")
```
...
```{r, echo=FALSE, layout="l-body-outset", eval=TRUE}
#system("cp files/dada2/figures/its18_plot_errorR_1.png include/dada2/its18_plot_errorR_1.png")
knitr::include_graphics("include/dada2/its18_plot_errorR_1.png")
```
...
*Total number of reads, total number of ASV, minimum and maximum ASVs, followed by the number of singletons, doubletons, etc.*
```{r, echo=FALSE, layout="l-body", eval=TRUE}
dada_pool_comp <- read.table("files/dada2/tables/its18_dada_compare.txt",
                        header = TRUE, sep = "\t", row.names = 1)
knitr::kable(dada_pool_comp)
```
...
*Total number of reads by sample.*
```{r, echo=FALSE, layout="l-body", eval=TRUE}
dada_pool_reads <- read.table("files/dada2/tables/its18_dada_compare_final_reads.txt",
                        header = TRUE, sep = "\t", row.names = 1)
knitr::kable(dada_pool_reads)
```
...
*Total number of ASVs by sample.*
```{r, echo=FALSE, layout="l-body", eval=TRUE}
dada_pool_asvs <- read.table("files/dada2/tables/its18_dada_compare_final_asvs.txt",
                        header = TRUE, sep = "\t", row.names = 1)
knitr::kable(dada_pool_asvs)
```
</details>
...
```{r, echo=FALSE, layout="l-body-outset", eval=TRUE}
sum_2018 <- read.table("files/dada2/tables/its18_read_changes.txt",
                       header = TRUE)
datatable(sum_2018, width = "80%",
          extensions = 'Buttons', options = list(
            scrollX = TRUE,
            dom = 'Blfrtip',
            buttons = c('copy', 'csv', 'excel'),
...
The source code for this page can be accessed on GitHub by [clicking this link](https://github.com/sweltr/high-temp/blob/master/dada2.Rmd). Please note, that in order to process the data *and*  build the website, we needed to run the workflow and get the results. Then hard code the results and turn off the individual commands. So the raw file for this page is a bit messy---you have been warned.

## Data Availability {.appendix}

Raw fastq files available on figshare at XXXXXXXX. Trimmed fastq files (primers removed) available through the ENA under project accession number [PRJEB45074 (ERP129199)](https://www.ebi.ac.uk/ena/browser/view/PRJEB45074). Output files from this workflow available on figshare [XXXXXXXX.]().
