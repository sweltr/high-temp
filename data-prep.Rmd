...
pacman::p_load(tidyverse, DT, microbiome, taxa, 
...
In order to run this workflow, you either need to run the corresponding [DADA2 Workflow for 2018 16S rRNA](dada2-high-temp.html#s-rrna-data.html) or begin with the output from that workflow, `data_prep_ssu18_wf.rdata`. See the [Data Availability](data-availability.html) page for complete details.
...
```{r, echo=FALSE}
sam_tab <- read.table(
  "files/dada2/tables/ssu18_sample_seq_info.txt",
  header = TRUE, sep = "\t")
sam_tab[7:11] <- NULL

read_tab <- read.table(
  "files/dada2/tables/ssu18_read_changes.txt",
  header = TRUE,  sep = "\t")
read_tab[3:6] <- NULL
names(read_tab)[1] <- "Sample_ID"
joined_tab <- left_join(sam_tab, read_tab,
                        by = c("Sample_ID" = "Sample_ID"))
joined_tab_percent <- joined_tab
percent_change <- joined_tab$nonchim/joined_tab$raw %>% round(digits = 3)
joined_tab_percent$Change <- as.numeric(sprintf("%.3f", percent_change))
joined_tab_percent$New_ID <- base::paste(joined_tab_percent$Plot,
                                   joined_tab_percent$Depth, sep = "_D")
joined_tab_percent$New_ID <- base::paste(joined_tab_percent$New_ID,
                                   substr(joined_tab_percent$Treatment, 1,1), sep = "_")
joined_tab_percent$New_ID <- base::paste(joined_tab_percent$New_ID,
                                   joined_tab_percent$Temp, sep = "")
joined_tab_percent$New_ID <- base::paste(joined_tab_percent$New_ID,
                                   joined_tab_percent$Pairing, sep = "")
joined_tab_percent <- joined_tab_percent[,c(10,2,5,3,4,6,7,8,9,1)]
colnames(joined_tab_percent) <- c("Sample_ID", "Plot", "Depth",
                                  "Treatment", "Temp", "Pair", "input",
                                   "nochim", "Remain", "FastqID")
```
...
datatable(joined_tab_percent, width = "100%", escape = FALSE,
          ...
          elementId = "po4ycztlg0tzswm5sssm",
...
temp_temp <- as.character(joined_tab$Temp)
p <- ggplot(joined_tab, aes(x = temp_temp, y = nonchim,
                              color = temp_temp)) +
  geom_point(size = 3) + scale_colour_manual(values = wong_pal)
p <- p + labs(x = "Temp (C)", y = "Final Read Count")
p
ggsave("include/data-prep/ssu18_samp_summary_plot.png", p, width = 7, height = 3)
```

```{r, echo=FALSE, layout="l-body-outset", warning=FALSE, fig.height=2.5, eval=TRUE}
knitr::include_graphics("include/data-prep/ssu18_samp_summary_plot.png")
```
...
example_sam <- "P07_D00-010_W4D"
qpair <- samdf[example_sam,]$PAIR
qgroup <- samdf %>% dplyr::filter(PAIR == qpair)
```

For example, `r example_sam` is the sample heated at +`r samdf[example_sam,]$TEMP`C from plot `r samdf[example_sam,]$PLOT` and a depth of `r samdf[example_sam,]$DEPTH`cm. This sample is in group `r qpair` along with `r qgroup$SamName`.
...
datatable(merge_tab2, width = "100%", escape = FALSE,
          ...
          elementId = "y2yczq8hn32js8mwqmp6",
          extensions = 'Buttons', options = list(
            scrollX = TRUE,
            dom = 'Blfrtip',
            buttons = c('copy', 'csv', 'excel'),
    ...
    formatRound(columns=c("reads_lost"), digits = 2) %>%
...
```{r}
asv_tab <- as.data.frame(otu_table(ps_work))
...
write_csv(samp_data, "files/data-prep/tables/ssu18_sam_tab_mc.csv")
```
...
```{r}
amp_asv  <- data.frame(otu_table(ps_amp))
amp_asv <- amp_asv %>% tibble::rownames_to_column("OTU")
```

#### Taxonomy data

```{r}
amp_tax  <- data.frame(tax_table(ps_amp))
amp_tax <- amp_tax %>% tibble::rownames_to_column("OTU")
amp_tax$ASV_SEQ <- NULL
colnames(amp_tax)[colnames(amp_tax) == "ASV_ID"] <- "Species"
```
...
```{r}
amp_asv_tax <- left_join(amp_asv, amp_tax, by = "OTU")
```

### Create the ampvis2 object

```{r}
amp_data <- amp_load(amp_asv_tax, metadata = amp_meta_1, tree = ps_work_tree)
```
...
saveRDS(amp_data, "files/data-prep/rdata/ssu18_amp_data.rds")
saveRDS(samp_data, "files/data-prep/rdata/ssu18_samp_data.rds")
saveRDS(ps_amp, "files/data-prep/rdata/ssu18_ps_amp.rds")
saveRDS(obj$data$asv_counts, "files/data-prep/rdata/ssu18_asv_counts.rds")
gdata::keep(obj, samp_data, sure = TRUE)
save.image("files/data-prep/rdata/ssu18_taxmap.rdata")
```
...
```{r, echo=FALSE}
sam_tab <- read.table(
  "files/dada2/tables/its18_sample_seq_info.txt",
  header = TRUE, sep = "\t")
sam_tab[7:11] <- NULL

read_tab <- read.table(
  "files/dada2/tables/its18_read_changes.txt",
  header = TRUE,  sep = "\t")
names(read_tab)[1] <- "Sample_ID"
read_tab[3:8] <- NULL
joined_tab <- left_join(sam_tab, read_tab,
                        by = c("Sample_ID" = "Sample_ID"))
joined_tab_percent <- joined_tab
percent_change <- joined_tab$nonchim/joined_tab$raw %>%
  round(digits = 3)
joined_tab_percent$Change <- as.numeric(sprintf("%.3f", percent_change))
joined_tab_percent$New_ID <- base::paste(joined_tab_percent$Plot,
                                   joined_tab_percent$Depth, sep = "_D")
...
datatable(joined_tab_percent, width = "100%", escape = FALSE,
          ...
          caption = htmltools::tags$caption(
            ...
            read changes at start and end of DADA2 workflow. Use the buttons to
            navigate through the table or download a copy.')),
          elementId = "33fgpgd1k5ugbiw74hvx",
          extensions = 'Buttons', options = list(
            scrollX = TRUE,
            dom = 'Blfrtip',
            buttons = c('copy', 'csv', 'excel'),
...
p <- p + labs(x = "Temp (C)", y = "Final Read Count")
p
ggsave("include/data-prep/its18_samp_summary_plot.png", p, width = 7, height = 3)
```

```{r, echo=FALSE, layout="l-body-outset", warning=FALSE, fig.height=2.5, eval=TRUE}
knitr::include_graphics("include/data-prep/its18_samp_summary_plot.png")
```
<small>Final read count by depth. Each point represents a differnt sample. Note the Negative control is not included.</small>
...
We have a total of **`r length(unique(sample_name))`** samples, **`r length(unique(pair))`** sample pairs, from **`r length(unique(plot))`** plots, and **`r length(unique(depth))`** different depths. There are **`r length(unique(treatment))`** treatments corresponding to **`r length(unique(temp))`** different temperature regimes.
...
datatable(merge_tab2, width = "100%", escape = FALSE,
          ...
          caption = htmltools::tags$caption(
            style = 'caption-side: bottom; text-align: left;',
            'Table: ', htmltools::em('Total reads & ASVs by sample.')),
          elementId = "gakgruoovivy4mz28ah2",
          extensions = 'Buttons', options = list(
            scrollX = TRUE,
            dom = 'Blfrtip',
            buttons = c('copy', 'csv', 'excel'),
...
We have phyloseq objects but we also want to make a `taxmap` object so we can use the [metacoder](https://github.com/grunwaldlab/metacoder) [@foster2017metacoder] and [taxa](https://github.com/ropensci/taxa) packages. We will jump back and forth between between different tools and packages because each has functionality not available in the others. That said, each package also requires a specially formatted *object* to work with.
...
write_csv(samp_data, "files/data-prep/tables/its18_sam_tab_mc.csv")
```
...
Now we have a taxmap object *and* a phyloseq object from the taxmap. Hopefully this will make it easier to generate data from one and import it into the other.
...
saveRDS(amp_data, "files/data-prep/rdata/its18_amp_data.rds")
saveRDS(sam_tab, "files/data-prep/rdata/its18_samp_data.rds")
saveRDS(ps_amp, "files/data-prep/rdata/its18_ps_amp.rds")
gdata::keep(obj, sam_tab, sure = TRUE)
save.image("files/data-prep/rdata/its18_taxmap.rdata")
```
...
You can find the source code for this page by [clicking this link](https://github.com/sweltr/high-temp/blob/master/data-prep.Rmd).
