...
```{r, echo=FALSE, eval=TRUE, results='markdown'}
...
knitr::kable(ssu18_perm_pvalues)
```
...
```{r, echo=FALSE, eval=TRUE}
tmp_perm_tests <- ssu18_perm_pvalues[ssu18_perm_pvalues$data_set == 'ssu18_ps_work_otu',]
```
...
```{r, echo=FALSE, eval=TRUE}
tmp_perm_tests <- ssu18_perm_pvalues[ssu18_perm_pvalues$data_set == 'ssu18_ps_pime_otu',]
```
...
```{r, echo=FALSE, eval=TRUE}
tmp_perm_tests <- ssu18_perm_pvalues[ssu18_perm_pvalues$data_set == 'ssu18_ps_perfect_otu',]
```
...
3. Next, calculate beta dispersion using the `betadisper` function from the `vegan` package.
4. Then, use the function `permutest` to run a Permutation test for homogeneity of multivariate dispersions.
5. If the beta dispersion tests are not significant we will run a PERMANOVA using `adonis` (since PERMANOVA assumes equal dispersion), otherwise we will use Analysis of Similarity (ANOSIM).
...
knitr::kable(its18_perm_pvalues)
```
...
```{r, echo=FALSE, eval=TRUE}
tmp_perm_tests <- its18_perm_pvalues[its18_perm_pvalues$data_set == 'its18_ps_work_otu',]
```
...
```{r, echo=FALSE, eval=TRUE}
tmp_perm_tests <- its18_perm_pvalues[its18_perm_pvalues$data_set == 'its18_ps_pime_otu',]
```
...
```{r, echo=FALSE, eval=TRUE}
tmp_perm_tests <- its18_perm_pvalues[its18_perm_pvalues$data_set == 'its18_ps_perfect_otu',]
```
