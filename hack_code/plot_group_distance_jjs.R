...
{
    group_distance <- tmp_t1$res_group_distance
    group <- tmp_t1$group
    if (tmp_t1$measure %in% c("wei_unifrac", "unwei_unifrac", "bray", 
        "jaccard")) {
        titlename <- switch(tmp_t1$measure, wei_unifrac = "Weighted Unifrac", 
            unwei_unifrac = "Unweighted Unifrac", bray = "Bray-Curtis", 
            jaccard = "Jaccard")
        ylabname <- paste0(titlename, " distance")
    }
    else {
        ylabname <- tmp_t1$measure
    }
